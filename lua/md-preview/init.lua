...
function M.setup(opts)
  for k, v in pairs(opts or {}) do M.opts[k] = v end
  ...
  elseif uname.sysname:find("Windows") or uname.sysname:find("MINGW")
      or uname.sysname:find("MSYS") then
    M.state.platform = nil
    notify.warn("md-preview: native Windows is not supported (sysname="
      .. uname.sysname .. ")")
  else
  ...
  if vim.fn.executable("mdp") == 0 then
    ...
    notify.warn("mdp not found. Run :MdPreviewInstall, or add "
      .. "`build = \":MdPreviewInstall\"` to your plugin spec "
      .. "(or `build = \"curl -fsSL " .. install.SCRIPT_URL .. " | sh\"`).")
  end
end
...
function M.open(theme)
  ...
  M.state.job_id = vim.fn.jobstart(
    { "mdp", "serve", file, tostring(M.opts.port), theme },
    {
      stdin = "pipe",
